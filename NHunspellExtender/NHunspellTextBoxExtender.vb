...
''' <summary>
''' This class defines the event handler arguments for a language change.
''' </summary>
Public Class LanguageChangedEvent
    Inherits EventArgs

    Property NewLanguage As String
End Class

''' <summary>
''' This class defines the event handler arguments for the completion of
''' a custom paint.
''' </summary>
Public Class CustomPaintCompletedEvent
    Inherits EventArgs
...
Public Class NHunspellTextBoxExtender
    ...
    Const DefaultLanguage As String = "UK English"
...
#Region "Private Classes"
    ...
    Private Class CustomPaintTextBox
        ...
        Public Event CustomPaintComplete(ByVal sender As Object, ByVal e As CustomPaintCompletedEvent)
...
GetNextLine:
            ...
            Dim e As New CustomPaintCompletedEvent
            e.Milliseconds = Now.Subtract(startTime).TotalMilliseconds
            RaiseEvent CustomPaintComplete(parentTextBox, e)
        End Sub
        ...
        Private Sub TextBoxBase_ClientSizeChanged(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            'Create a bitmap with the same dimensions as the textbox
            If tempTextBox.Width > 0 And tempTextBox.Height > 0 Then
...
#Region "New and CanExtend Methods"
    ...
    Public Sub New()
        ...
        USdic = ""
        USaff = ""
        InstallLanguage("UK English", "en_UK", My.Resources.en_UK, My.Resources.en_UK_dic, USdic, USaff)
...
#End Region
    ...
    Public Sub InstallLanguage(ByVal language As String, ByVal filename As String, ByVal sourceAff As Byte(), ByVal sourceDic As Byte(), ByRef USdic As String, ByRef USaff As String)
        ...
        If regKey Is Nothing Then
            ...
            If Not File.Exists(USaff) Then
                ...
                Catch ex As Exception
                    MessageBox.Show("Error writing " & USaff & " file!" & vbNewLine & ex.Message)
                End Try
            End If
            ...
            If Not File.Exists(USdic) Then
                ...
                Catch ex As Exception
                    MessageBox.Show("Error writing " & USdic & " file!" & vbNewLine & ex.Message)
                End Try
            End If

            Dim paths() As String = {USaff, USdic}
            regKeyLanguage.SetValue(language, paths, RegistryValueKind.MultiString)

            Dim languages() As String = {language}
        ...
        Else
            ...
            If regKeyLanguage.GetValue(defaultLanguage) Is Nothing Then
                ...
                paths = TryCast(regKeyLanguage.GetValue(language), String())
...
#Region "Properties"
    ...
    Public Event LanguageChanged(ByVal sender As Object, ByVal e As LanguageChangedEvent)
    ...
    Public Property Language() As String
        ...
        Set(ByVal value As String)
            ...
            If raiseChangeEvent Then
                Dim newLanguage As New LanguageChangedEvent()

                newLanguage.NewLanguage = _Language

                RaiseEvent LanguageChanged(Me, newLanguage)
            End If
        End Set
    End Property
...
#Region "CustomEvents"
    ...
    Public Sub OnCustomPaintComplete(ByVal sender As Object, ByVal e As CustomPaintCompletedEvent)
        RaiseEvent CustomPaintComplete(sender, e)
    End Sub
#End Region
...
#End Region
    ...
    Private Sub MyLanguageChanged(ByVal sender As Object, ByVal e As LanguageChangedEvent)
        CType(sender, NHunspellTextBoxExtender).SetLanguage(e.NewLanguage)
    End Sub
...
#Region "ContextMenu"
    ...
    Private Sub ReplaceWord(ByVal OriginalWord As String, ByVal NewWord As String)
        ...
        Dim Position = NativeMethods.GetScrollPos(ownerTextBox.Handle, SBS_VERT)

        'Disable redraw
        NativeMethods.SendMessage(ownerTextBox.Handle, WM_SETREDRAW, New IntPtr(CInt(False)), IntPtr.Zero)
        ...
        If (NativeMethods.SetScrollPos(ownerTextBox.Handle, SBS_VERT, Position, True) <> -1) Then
            NativeMethods.PostMessageA(ownerTextBox.Handle, WM_VSCROLL, SB_THUMBPOSITION + &H10000 * Position, Nothing)
        End If

        'Enable redraw
        NativeMethods.SendMessage(ownerTextBox.Handle, WM_SETREDRAW, New IntPtr(CInt(True)), IntPtr.Zero)
    ...
    Private Sub AddWordToDictionary(ByVal WordToAdd As String)
        If Not boolDisableAddWordPrompt Then
            ...
            result = MessageBox.Show("This will add the word " & Chr(34) & WordToAdd & Chr(34) & "." & vbNewLine & vbNewLine & "Are you sure?",
        ...
        For i = 0 To UBound(myControls)
            If TypeOf myControls(i) Is TextBoxBase Then
                With CType(myControls(i), TextBoxBase)
                    'Get original scroll position
                    Dim Position = NativeMethods.GetScrollPos(.Handle, SBS_VERT)

                    'Disable redraw
                    NativeMethods.SendMessage(.Handle, WM_SETREDRAW, New IntPtr(CInt(False)), IntPtr.Zero)
                    ...
                    'Reset scroll position
                    If (NativeMethods.SetScrollPos(.Handle, SBS_VERT, Position, True) <> -1) Then
                        NativeMethods.PostMessageA(.Handle, WM_VSCROLL, SB_THUMBPOSITION + &H10000 * Position, Nothing)
                    End If

                    'Enable redraw
                    NativeMethods.SendMessage(.Handle, WM_SETREDRAW, New IntPtr(CInt(True)), IntPtr.Zero)
...
#Region "Change Language"
    ...
    Public Sub AddCustomWords(ByVal ArrayOfWords As String())
        If Not myNHunspell Is Nothing Then
            For Each Word In ArrayOfWords
                myNHunspell.Add(Word)
            Next
        End If
    ...
    Public Sub RemoveLanguage(ByVal LanguageToRemove As String)
        ...
        If regKey.GetValue("Default") = LanguageToRemove Then
            If regKey.GetValue("UK English") IsNot Nothing Then
                SetLanguage("UK English")
            Else
    ...
    Private Sub ResetLanguages()
        ...
        Dim paths() As String = TryCast(regKey.GetValue("UK English"), String())
        ...
        If regKey.GetValue("UK English") Is Nothing Then
            ...
            If Not File.Exists(USaff) Then
                ...
                Catch ex As Exception
                    MessageBox.Show("Error writing en_UK.aff file!" & vbNewLine & ex.Message)
                End Try
            End If
            ...
            If Not File.Exists(USdic) Then
                ...
                Catch ex As Exception
                    MessageBox.Show("Error writing en_UK.dic file!" & vbNewLine & ex.Message)
                End Try
            End If
        ...
        _Language = "UK English"
        regKey.SetValue("Default", "UK English")
        regKey.SetValue("English", paths, RegistryValueKind.MultiString)
        regKey.SetValue("LanguageList", {"UK English"}, RegistryValueKind.MultiString)
        regKey.Close()
